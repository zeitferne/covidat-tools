...
def processfile(dts: Set[date], fname: Path, azr: pd.DataFrame, writer: csv.DictWriter):
    ...
    if fname.name.endswith(".xlsx"):
        ...
        msk = pd.isna(veasp["Verwendung"]) & veasp[
            "Zulassungsnummer"
        ].str.match("EU/.+[-,]")
        veasp.loc[msk, "Zulassungsnummer"] = veasp.loc[
            msk, "Zulassungsnummer"
        ].str.replace("[-,][^/]+$", "", regex=True, n=1)
        veasp = veasp.merge(
            azr[["Zulassungsnummer", "Verwendung"]],
            "left",
            on="Zulassungsnummer",
        ...
        # display(veasp)
        veasp["Status"].replace(
            "Nicht verfügbar", "nicht verfügbar", inplace=True
        )
        veasp["Verwendung"] = veasp["Verwendung"].combine_first(
            veasp["Verwendung_y"]
        )
        ...
        if len(naveasp) > 0:
            raise ValueError(
                str(fname)
                + ": Not found in AZR: "
                + naveasp[["Zulassungsnummer", "Name"]].to_csv(
                    sep=";", index=False
                )
            )
        ...
        try:
            data = (
                veasp[["Verwendung", "Status", "Zulassungsnummer", "Name"]]
                .groupby(["Zulassungsnummer", "Verwendung"])
                .agg({"Name": "first", "Status": agg_status})
            ).reset_index()
    ...
    if False:  # Group by name
        ...
        try:
            data = data.groupby([dkey, data["Verwendung"]]).agg(
                {"Status": agg_status}
            )
        except StopIteration:
    ...
    with open(outname, "w", encoding="utf-8", newline="\n") as outfile:
        ...
        for fname in sorted(
            ...
            try:
                ...
                traceback.print_exc()
                print("The previous exception occured during processing of", fname, file=sys.stderr, flush=True)
