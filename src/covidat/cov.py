...
def load_bev(*, ages_age_groups=True) -> pd.DataFrame:
    ...
    if ages_age_groups:
        ag_col = "AltersgruppeID"
        result[ag_col] = result["AltersgruppeID5"].map(
            lambda ag5: 1 if ag5 == 1 else 10 if ag5 >= 18 else (ag5 + 2) // 2
        )
    else:
        ag_col = "AgeFrom"
        result[ag_col] = (result["AltersgruppeID5"] - 1) * 5
    return result.groupby(["Datum", "BundeslandID", ag_col, "Geschlecht"])["AnzEinwohner"].sum()


def bev_to_daily(
    statbev: pd.DataFrame, minyear: int = 2020, maxyear: int | None = None, *, add_at_sum=True
) -> pd.DataFrame:
    ...
    catcols = [c for c in pandembev.index.names if c != "Datum"]
    pandembev = pandembev.groupby(catcols).apply(
        lambda s: s.reset_index(catcols, drop=True).to_timestamp().resample("D").interpolate()
    )
    if add_at_sum:
